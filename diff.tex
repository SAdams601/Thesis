...
%DIF ADD thesis.tex            Sun Aug 12 13:14:44 2018
...
\DIFaddbegin \DIFadd{The introductory chapter of this thesis will begin with a brief discussion of functional programming and the Haskell programming language. Next will be a very high level overview of refactoring with a focus on refactoring functional programming languages in particular. Finally this chapter concludes with an outline of the remainder of this thesis.
}
...
\DIFdelend \DIFaddbegin \DIFadd{Chapter~\ref{chp:generalImp}}\DIFaddend .
	...
	\item The design and implementation of the ``monadification'' refactoring, see \DIFdelbegin \DIFdel{chapter}\DIFdelend \DIFaddbegin \DIFadd{Chapter}\DIFaddend ~\ref{chp:monadification}. Monadification is the process of making a program work over a monadic type rather than a pure one. This refactoring supports the evolution of pure programs to effectful ones.
\end{itemize}


%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \chapter{\DIFadd{Related Work}}
\label{chp:related}
...
\section{\DIFadd{Refactoring Tools in Modern IDEs}}\label{ideTools}
...
\section{\DIFadd{Refactoring Tools for functional languages}}\label{funcTools}
...
\subsection{\DIFadd{Haskell Tools Refact}}
...
\subsection{\DIFadd{Wrangler}}\label{wranglerOne}
...
\subsection{\DIFadd{RefactorErl}}\label{refactorErl}
...
\section{\DIFadd{Refactoring to introduce parallelism}}\label{refacParallel}
...
\subsection{\DIFadd{Cost-Directed Parallel Refactoring}}
...
\section{\DIFadd{ApplicativeDo}}\label{applicativeDo}
...
\section{\DIFadd{Program transformations}}\label{typeTrans}
...
\subsection{\DIFadd{Type and transform systems}}
...
\subsection{\DIFadd{Automatic Monadification}}\label{erwigMonad}
...
\subsection{\DIFadd{Data Type Transformations}}\label{dtt}
...
\subsection{\DIFadd{Generic Refactorings}}
...
\section{\DIFadd{Engineering refactoring tools}}\label{engineeringRefac}
...
\subsection{\DIFadd{Implementing Wrangler}}
...
\subsection{\DIFadd{Refactoring LISPs}}
...
\DIFdelbegin \section{\DIFdel{Underlying technologies}}
%DIFAUXCMD
...
\DIFadd{Next this thesis will begin to describe its major contribution: the design and implementation of data-driven refactorings. The next chapter will begin the discussion of what is a data-driven refactoring and some simple refactoring designs.
}
...
\DIFdelbegin \subsection{\DIFdel{Introducing a Type Synonym}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Introducing a Type Synonym}}\DIFaddend \label{introSyn}
...
\section{\DIFdelbegin \DIFdel{Generalising Maybe}\DIFdelend \DIFaddbegin \DIFadd{Generalisation}\DIFaddend }
\label{genMaybe}
...
\subsection{\DIFadd{Generalising Maybe}}

\DIFaddend This section describes a refactoring that rewrites programs of type \texttt{Maybe a} to use the \texttt{MonadPlus} type class or, if possible, the \texttt{Monad} type class. \DIFaddbegin \texttt{\DIFadd{Maybe}} \DIFadd{is a data type whose declaration is shown in Figure~\ref{maybeDecl}. }\texttt{\DIFadd{Maybe}} \DIFadd{represents a computation that can fail or return a value of type }\texttt{\DIFadd{a}}\DIFadd{. When a computation has failed }\texttt{\DIFadd{Maybe}} \DIFadd{represents this ``result'' with the }\texttt{\DIFadd{Nothing}} \DIFadd{constructor. When a calculation succeeds then }\texttt{\DIFadd{Maybe}} \DIFadd{returns the result wrapped with the }\texttt{\DIFadd{Just}} \DIFadd{constructor. }\texttt{\DIFadd{Maybe}} \DIFadd{is a commonly used type in Haskell programs, it also implements many common type classes including }\texttt{\DIFadd{Monad}} \DIFadd{and }\texttt{\DIFadd{MonadPlus}}\DIFadd{.
}\DIFaddend 

\DIFaddbegin \DIFadd{Monads are a data type used to represent ``composable computation descriptions''~\mbox{%DIFAUXCMD
\citep{haskellWikiMonad}}\hspace{0pt}%DIFAUXCMD
. Haskell uses monads to augment pure computations with features that other languages would allow as side effects such as state or I/O. Haskell has a built in monad type class, whose declaration is shown in Figure~\ref{monadTC}. Instances of monad need to implement two functions: }\texttt{\DIFadd{return}} \DIFadd{which will create a monadic computation that will produce its parameter and }\texttt{\DIFadd{>>=}} \DIFadd{(which is pronounced bind). Bind's first argument is just some monadic value with a result of type }\texttt{\DIFadd{a}} \DIFadd{and its second argument is a function that takes in a value of type }\texttt{\DIFadd{a}} \DIFadd{and returns another monadic value that computes a result of type }\texttt{\DIFadd{b}}\DIFadd{. Bind computes the result of type }\texttt{\DIFadd{a}} \DIFadd{from the monadic value it was given in its first argument and passes that value to the function it received as its second argument. This description of bind only applies when its first argument is capable of computing some result or when the monad instance does not need to perform any additional tasks. This is the great power of monads as an abstraction, because bind can be implemented in almost any way}\footnote{\DIFadd{All instances of monad must abide by the three monad laws~\mbox{%DIFAUXCMD
\citep{monadLaws}}\hspace{0pt}%DIFAUXCMD
}} \DIFadd{it can act as a way to specify how a particular set of computations are performed. In the case of }\texttt{\DIFadd{Maybe}} \DIFadd{bind is used to cause a chain of computations return }\texttt{\DIFadd{Nothing}} \DIFadd{if one of the steps of computation fail.
}
...
data Maybe a = Nothing
                 | Just a
\end{lstlisting}
\DIFaddendFL \caption{The \texttt{Maybe} data type \DIFdelbeginFL \DIFdelFL{definition and }\texttt{\DIFdelFL{MonadPlus}} %DIFAUXCMD
\DIFdelFL{instance }\DIFdelendFL declaration\DIFdelbeginFL \DIFdelFL{, and the }\texttt{\DIFdelFL{MonadPlus}} %DIFAUXCMD
\DIFdelFL{class definition}\DIFdelendFL .}
\DIFaddbeginFL \label{maybeDecl}
\end{figure}
...
\DIFaddendFL \label{maybeMonadPlus}
\end{figure}

\DIFaddbegin \DIFadd{Monads are a powerful abstraction but it is also useful to further categorise monads that exhibit different properties. Haskell comes with type classes, that inherit from }\texttt{\DIFadd{Monad}}\DIFadd{, and that categorise additional features monads can have, such as failure (}\texttt{\DIFadd{MonadFail}}\DIFadd{) or failure and recovery (}\texttt{\DIFadd{MonadPlus}}\DIFadd{)~\mbox{%DIFAUXCMD
\citep{typeclassopedia}}\hspace{0pt}%DIFAUXCMD
. }\texttt{\DIFadd{MonadPlus}} \DIFadd{is a type class that represents the type of computations that may fail and also provide some way of choosing between possibly failed computations. }\DIFaddend Figure~\ref{maybeMonadPlus} \DIFdelbegin \DIFdel{contains }\DIFdelend \DIFaddbegin \DIFadd{shows }\DIFaddend the class declaration for \texttt{MonadPlus} \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend \texttt{Maybe}\DIFaddbegin \DIFadd{'s }\DIFaddend instance of that class. The \texttt{Maybe} type represents a computation that may return a value wrapped in the \texttt{Just} constructor or may fail (represented by the \texttt{Nothing} constructor). The \texttt{MonadPlus} class is a \DIFdelbegin \DIFdel{typeclass }\DIFdelend \DIFaddbegin \DIFadd{type class }\DIFaddend for monads that also have \DIFdelbegin \DIFdel{a monoidal structure~}\footnote{\DIFdel{A monoid is a semigroup whose associative binary operation has an identity element.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The }\texttt{\DIFdel{MonadPlus}} %DIFAUXCMD
\DIFdel{class helps generalise monads that contain }\DIFdelend some concept of failure and choice. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{All instances of }\texttt{\DIFadd{MonadPlus}} \DIFadd{have an }\DIFaddend \texttt{mzero} value\DIFaddbegin \DIFadd{, which }\DIFaddend represents a failed computation\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, and an }\DIFaddend \texttt{mplus} \DIFaddbegin \DIFadd{function, which }\DIFaddend represents a way of making a \DIFaddbegin \DIFadd{left-biased }\DIFaddend "choice" between two computations that may or may not have failed~\citep{typeclassopedia}. 
...
instance Monad Maybe where
	...
	Nothing  >>= _ = Nothing
\end{lstlisting}
\DIFaddendFL \caption{\texttt{Maybe}'s monad instance definition}
\label{maybeMonad}
\end{figure}


\subsection{Refactoring Maybe to \DIFdelbegin \DIFdel{Monad}\DIFdelend \DIFaddbegin \DIFadd{MonadPlus}\DIFaddend } 
\DIFdelbegin %DIFDELCMD < \label{genMonad}
%DIFDELCMD < %%%
\DIFdelend 

\DIFdelbegin \DIFdel{This refactoring replaces }\DIFdelend \DIFaddbegin \texttt{\DIFadd{MonadPlus}} \DIFadd{is the obvious generalisation of }\DIFaddend \texttt{Maybe}\DIFdelbegin \DIFdel{-specific code with the more general }\DIFdelend \DIFaddbegin \DIFadd{. }\texttt{\DIFadd{Maybe}} \DIFadd{encodes failure using the }\texttt{\DIFadd{Nothing}} \DIFadd{constructor and its definition of }\texttt{\DIFadd{mplus}} \DIFadd{chooses to discard failed computations on the left in exchange for a possibly successful one. This behaviour is exactly what }\texttt{\DIFadd{MonadPlus}} \DIFadd{is designed to encapsulate. This section describes a refactoring for rewriting functions that use }\texttt{\DIFadd{Maybe}} \DIFadd{to use the }\texttt{\DIFadd{MonadPlus}} \DIFadd{type class instead.
}
...
\DIFadd{The function }\texttt{\DIFadd{showNat}} \DIFadd{takes a pure value and returns a }\texttt{\DIFadd{Maybe}}\DIFadd{, instead of taking in a }\texttt{\DIFadd{Maybe}} \DIFadd{and returning another }\texttt{\DIFadd{Maybe}} \DIFadd{as in the previous example. The more general version of }\DIFaddend \texttt{\DIFdelbegin \DIFdel{Monad}\DIFdelend \DIFaddbegin \DIFadd{showNat}\DIFaddend } \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{needs to be able to express the idea of failure that }\DIFaddend \texttt{MonadPlus} \DIFdelbegin \DIFdel{operations. 
This is done by recognizing when }\DIFdelend \DIFaddbegin \DIFadd{encodes with the }\texttt{\DIFadd{mzero}} \DIFadd{operation. 
}
...
\subsection{\DIFadd{Refactoring Maybe to Monad}}\label{genMonad}
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Refactoring Maybe to MonadPlus}} 
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 
...
\subsection{\DIFdelbegin \DIFdel{Refactoring lists to Hughes lists}%DIFDELCMD < }
%DIFDELCMD < 
...
\DIFdel{This refactoring }\DIFdelend \DIFaddbegin \DIFadd{Embeddable Types}} 
...
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Refactoring functions to use Hughes lists}}
\DIFaddend 
...
\subsection{Modifying the Type of \DIFdelbegin \DIFdel{a Parameter}\DIFdelend \DIFaddbegin \DIFadd{an Input}\DIFaddend }
...
The previous chapter described \DIFdelbegin \DIFdel{two refactorings, }\DIFdelend \DIFaddbegin \DIFadd{three refactorings, ``introducing a type synonym,'' ``}\DIFaddend generalising \texttt{Maybe}\DIFdelbegin \DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{,'' and the ``}\DIFaddend list to Hughes list refactorings\DIFaddbegin \DIFadd{''}\DIFaddend . This chapter will describe how refactorings are implemented in HaRe. Section~\ref{sec:genMaybeImp} describes the implementation of the generalising \texttt{Maybe} refactoring which was first covered in \DIFdelbegin \DIFdel{section}\DIFdelend \DIFaddbegin \DIFadd{Section}\DIFaddend ~\ref{genMaybe}. Section~\ref{sec:hughesListImp} covers the implementation of the \DIFaddbegin \DIFadd{``}\DIFaddend list to Hughes list\DIFaddbegin \DIFadd{'' }\DIFaddend refactoring (see \DIFdelbegin \DIFdel{section}\DIFdelend \DIFaddbegin \DIFadd{Section}\DIFaddend ~\ref{listToDlist}) and the embeddable type refactoring system that it is made with. The chapter also covers the enhancements that HaRe's API has undergone while developing the refactorings discussed in this thesis. Section~\ref{hareAPI} discusses the enhancements I made to HaRe's API.
...
In their 2008 functional pearl \DIFdelbegin \DIFdel{"Applicative programming with effects" }\DIFdelend \DIFaddbegin \DIFadd{``Applicative Programming with Effects,~\mbox{%DIFAUXCMD
...
\section{The \DIFdelbegin \DIFdel{applicative Typeclass}\DIFdelend \DIFaddbegin \DIFadd{Applicative Type Class}\DIFaddend }
...
\DIFdelbegin \subsection{\DIFdel{Implementation of the Generalising to Applicative Refactoring}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
...
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Checking the Preconditions}}
\DIFaddend 
...
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Constructing the Effectful Expression}}
\DIFaddend \begin{figure}[t]
\begin{math}
\begin{aligned}
...
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Building the Pure Expression}}
\DIFaddend 
...
\section{The Monad \DIFdelbegin \DIFdel{Typeclass}\DIFdelend \DIFaddbegin \DIFadd{type class}\DIFaddend }
\label{monTC}
...
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{\DIFdelbegin \DIFdel{Related Work}\DIFdelend \DIFaddbegin \DIFadd{Conclusion}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \label{chp:related}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{chp:conc}
\DIFaddend 

This \DIFdelbegin \DIFdel{chapter reviews current work in refactoring and other areas of the literature. The chapter is divided into several sections. This chapter will begin with a brief description of refactoring tools in general. The second section (\ref{funcTools}) will describe work done in the area of tooling for functional languages such as HLint~\mbox{%DIFAUXCMD
\citep{hlint} }\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\section{\DIFdel{Refactoring Tools in Modern IDEs}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
...
\citep{ides}}\hspace{0pt}%DIFAUXCMD
. These refactoringtools support some general (renaming, method extraction) and object-oriented specific refactorings (pushing/pulling methods up/down the object hierarchy). 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Refactoring Tools for functional languages}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{funcTools}
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdel{Code smell and refactoring tools are very closely related. Simplistically a code smell tool detects problems in a code base and a refactoring tool fixes them. If a tool can detect a problem why can't the same program fix them? HLint has a }\DIFdelend \DIFaddbegin \DIFadd{thesis has explored a new category of refactoring for functional programming languages. Data-driven refactorings are transformations that are }\textit{\DIFadd{driven}} \DIFadd{by the data types a program uses. The generalisation refactorings, for example, are motivated by a desire to allow code to be re-used in more places (e.g. the ``Maybe to MonadPlus'' refactoring) or even to generalise the way a program is evaluated as in the ``monad to applicative'' refactoring where the }\DIFaddend \texttt{\DIFdelbegin \DIFdel{-refactor}\DIFdelend \DIFaddbegin \DIFadd{Applicative}\DIFaddend } \DIFdelbegin \DIFdel{flag that will automatically apply the suggestions. However a single piece of code could have multiple smells, how would HLint choose which one to apply ? Also once a transformation was applied other hints may no longer be applicable.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{One of the powerful features of HLint is its customizability. An HLint (called }\DIFdelend \DIFaddbegin \DIFadd{type class does not force sequential evaluation of the arguments to apply (}\DIFaddend \texttt{\DIFdelbegin \DIFdel{hlint.yaml}\DIFdelend \DIFaddbegin \DIFadd{<*>}\DIFaddend }) \DIFdelbegin \DIFdel{configuration file added to the root of a project   will be detected by HLint and it will suggest both the default hints as well as the custom hints from that file.
Hints are very simple to write.
}\DIFdelend \DIFaddbegin \DIFadd{like bind (}\texttt{\DIFadd{>>=}}\DIFadd{), the monad operation, does.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{lstlisting}%DIFDELCMD < [caption={A simple hint from~\citep{hlint}}, label=lstHint]
%DIFDELCMD < - hint: {lhs: x !! 0, rhs: head x}
%DIFDELCMD < \end{lstlisting}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{These refactorings are all designed to help programmers redesign the data their programs are written over. It has been shown in the literature that as programmers make structural decisions the implementation of their programs technical debt can build up, and this debt can only be paid through rewriting and/or refactoring~(\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Listing~\ref{lstHint} contains the definition of a hint that detects the list index operator is being used to look up the 0th element of a list and suggests using }\texttt{\DIFdel{head}} %DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \section{\DIFadd{Summary of Contributions}}

\DIFadd{The primary artifacts of this thesis are my contributions to HaRe and its API. In particular the following contributions have been made:
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{lstlisting}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Haskell Tools Refact}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 
...
\citep{haskellToolsGit}}\hspace{0pt}%DIFAUXCMD
. There are six refactorings currently supported. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{itemize}
\DIFaddbegin \item \DIFadd{The design and implementation of a set of data-driven refactorings in HaRe. These refactorings are:
	}\begin{itemize}
		\DIFaddend \item \DIFdelbegin \DIFdel{Rename
}\DIFdelend \DIFaddbegin \DIFadd{Introduce a Type Synonym, Section~\ref{introSyn}
		}\DIFaddend \item \DIFdelbegin \DIFdel{Generate type signature
}\DIFdelend \DIFaddbegin \DIFadd{Generalise Maybe to MonadPlus/Monad, Section~\ref{maybeMonadPlus} and Chapter~\ref{chp:generalImp}
		}\DIFaddend \item \DIFdelbegin \DIFdel{Generate exports
}\DIFdelend \DIFaddbegin \DIFadd{List to Hughes List, Section~\ref{listToDlist} and chapter~\ref{chp:generalImp}
		}\DIFaddend \item \DIFdelbegin \DIFdel{Extract binding
}\DIFdelend \DIFaddbegin \DIFadd{Generalise Monad to Applicative, Chapter~\ref{chp:applicative}
		}\DIFaddend \item \DIFdelbegin \DIFdel{Inline binding
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Organize imports
}\DIFdelend \DIFaddbegin \DIFadd{Monadification	, Chapter~\ref{chp:monadification}	
	}\DIFaddend \end{itemize}
...
\citep{wrangler}}\hspace{0pt}%DIFAUXCMD
. Erlang is a functional programming language designed to be massively scalable andhighly fault tolerant~\mbox{%DIFAUXCMD
\citep{erlang}}\hspace{0pt}%DIFAUXCMD
. It was originally developed in 1986 by Joe Armstrong, Robert Virding, andMike Williams at the Computer Science Laboratory at Ericsson Telecom AB~\mbox{%DIFAUXCMD
\citep{erlangHistory}}\hspace{0pt}%DIFAUXCMD
...
\DIFdel{when it is called with one argument whereas the second template will match the same function with any number of arguments.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Composite refactorings are refactorings that are made up of multiple refactorings run, in sequence, one after the other. It can be challenging to develop composite refactorings if they arenot explicitly handled by the refactoring tool. The naive solution just chains refactorings together with the output from one refactoring in a composite refactoring becoming the input to the next refactoring. However, what if the second refactoring fails in a chain of four? composite refactoring definitions, without tool support, become filled with error handling code to manage the situation when one of the component refactorings fail. Wrangler defines a domain specific language that helps describe the various facets of a composite refactoring.
}%DIFDELCMD < 
...
g. "}\texttt{\DIFdel{aFunName}}%DIFAUXCMD
\DIFdel{"). The final parameter is generated by taking the second parameter and modifying it so that the name is in the corresponding "snake case" format }\DIFdelend \DIFaddbegin \item \DIFadd{An API for the creation of embeddable types refactorings, chapter~\ref{chp:generalImp}
}\item \DIFadd{Enhancements and additions to HaRe's API, see Chapter~\ref{chp:generalImp}. The API was extended while developing the refactorings mentioned above.
}\begin{itemize}
	\item \DIFadd{Functions that perform high level, common transformations, that many refactorings must do }\DIFaddend (e.g. \DIFdelbegin \DIFdel{"}\texttt{\DIFdel{a\_fun\_name}}%DIFAUXCMD
\DIFdel{")
	. 
}%DIFDELCMD < 
...
\citep{rotor}}\hspace{0pt}%DIFAUXCMD
. Language features of OCaml provide some unique challenges for a refactoring tool. 
	In OCaml one module may be included in another so that , for example, when renaming the function }\texttt{\DIFdel{f}} %DIFAUXCMD
...
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{wrapping a syntax element in parentheses)
	}\item \DIFadd{Functions for retrieving particular syntax elements from the abstract syntax tree of }\DIFaddend an \DIFdelbegin \DIFdel{"industrial strength" version of the OCaml standard library as a test bed for testing the refactoring tool.
 }\DIFdelend \DIFaddbegin \DIFadd{entire module such as getting the body of a function based on a position. 
	}\item \DIFadd{Functions that aid in working with ghc-exactprint and the parsed GHC abstract syntax tree's annotations. These functions provide functionality such as changing the location of a syntax element relative to the prior syntax element or adding new annotations to a syntax element.
}\end{itemize}	  
\end{itemize}
\DIFaddend 

\DIFdelbegin \section{\DIFdel{Novel applications of Refactoring}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{otherTools}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section{\DIFadd{The GHC Tooling Ecosystem}}
...
\DIFdelbegin \DIFdel{Though automated refactoring was originally developed for the object-oriented language Smalltalk~\mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Spreadsheet refactoring}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{Spreadsheets are a widely used tool in industry, with 13 million end users that are estimated to }\DIFdelend \DIFaddbegin \DIFadd{I began working on HaRe when the latest version of GHC was 7.6 (At the time of writing it is 8.4.3). At this point all comments, whitespace information, and the positioning of many special characters were completely eliminated from the parsed abstract syntax. To preserve the layout of source files HaRe had to generate a separate tree structure that contained all the tokens that were not represented by the AST and some whitespace information. When it came time to print the modified source the AST and this token tree needed to }\DIFaddend be \DIFdelbegin \DIFdel{"programming" with spreadsheets or databases in 2012; this figure dwarfs 3 million professional programmers at that time~\mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{combined for the output to be formatted correctly. 
}\DIFaddend 

\DIFdelbegin \DIFdel{BumbleBee is a refactoring tool whose grammar is built from the grammer for Excel formulas~\mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{Finally BumbleBee allows for cells to be referenced in the form ${i,j}$ and cells can be connected into groups with the "$...$" operator. This enables the following transformation rule that replaces a chain of sums to be replaced with a single call to $SUM$}\DIFdelend \DIFaddbegin \DIFadd{haskell-ide-engine}\footnote{\url{https://github.com/haskell/haskell-ide-engine}} \DIFadd{so that a Haskell environment can be easily incorporated into many different editors and IDEs and tool developers only have to target the ide engine plugin system. The future of Haskell tooling is looking very bright! And in particular this makes adding HaRe to other IDEs that much easier}\DIFaddend . 

\DIFdelbegin \begin{displaymath} \DIFdel{\{i,j\} + ... + \{m,n\} \leftrightarrow SUM(\{i,j\}:\{m,n\}) }\end{displaymath}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Challenges of Working with the GHC API}}
\DIFaddend 

\DIFdelbegin \DIFdel{The goal of this syntax is to allow user familiar with Excel but not another programming language to be able to create their own translations. BumbleBee is implemented as an add on for Excel so users can work with it in a familiar environment.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Wiki refactoring}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Refactoring a wiki is a surprisingly difficult task. Beyond just the content of an article there is also the history of changes to that article and }\DIFdelend \DIFaddbegin \DIFadd{Working with the GHC API is not without its challenges however. The structure of }\DIFaddend the \DIFdelbegin \DIFdel{"talk" page where users can discuss about the content of an article. A refactoring needs to preserve two key properties of the wiki, authorship independence and readership independence. Authorship independence means that a refactoring will preserve the attribution of content to the original author(s).
Readership independence implies that a refactoring will not alter the content of }\DIFdelend \DIFaddbegin \DIFadd{abstract syntax is very complex and spread across several modules. The core types of }\DIFaddend the \DIFdelbegin \DIFdel{wiki, a refactoring will only change how content is distributed throughout the wiki. }%DIFDELCMD < 
...
. Mind mapping is a method of visualizing the relationships between pieces of  hierarchical information. WikiWhirl represents a wiki as a mind map and users can then refactor the wiki using this view (see figure~\ref{wikiwhirl}). 
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Both wikis and spreadsheets are common data formats that deal with the same "smell" and quality issues that software projects experience. The difference between software development and these applications is that the users of the latter will not necessarily be technology professionals. Refactoring tools for wikis and spreadsheets have to be usable by laypeople not just software engineers}\DIFdelend \DIFaddbegin \DIFadd{tree are represented by many constructors, the expression type for example, }\texttt{\DIFadd{HsExpr}}\DIFadd{, contains 51 different constructors as of GHC 8. This amount of complexity makes a generic programming library a mandatory prerequisite for working with the abstract syntax tree}\DIFaddend . 

\DIFdelbegin \section{\DIFdel{Refactoring to introduce parallelism}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{refacParallel}
%DIFDELCMD < 
...
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Fortunately for tool builders the number of types and constructors that represent the vast majority of ``standard'' Haskell code is much more limited. However, determining which constructors you need to target for any given transformation can be very tedious: in }\DIFaddend the \DIFdelbegin \texttt{\DIFdel{parList rdeepseq}} %DIFAUXCMD
...
\DIFdel{is the strategy the fully evaluates its argument. 
  }\DIFdelend \DIFaddbegin \DIFadd{author's experience this process involves looking at numerous printouts of abstract syntax trees. 
}\DIFaddend 

\DIFdelbegin \DIFdel{The refactored program in figure~\ref{eulerPar1} is highly parallel but not very efficient because the parallelism is too fine grained. Another refactoring can help in this case instead of sparking every element of a list another strategy can be introduced, one that separates the list into "chunks" and each of the chunks of the list is executed in parallel. This refactoring adds an additional argument to the function that determines how many chunks the list will be split into,as seen in figure~\ref{eulerChunk}. }%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Cost-Directed Parallel Refactoring}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 
...
describes a methodology to introduce algorithmic skeletons into Erlang programs using the Erlang refactoring tool Wrangler. In addition to introducing a skeleton this work provides cost models that estimate the performance of the program after adding each skeleton. 
This estimate helps a programmer make an informed decision about what parallelisation strategy is the best for a particular program. 
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Parallelisation can be tedious anddifficult to do which makes it a good candidate for tool assistance. A refactoring tool can guide a programmer through the process of parallelisation. Much like how the data-driven refactorings have multiple small changes are required before the entire process can be considered ``finished'' changing a program to run in parallel is also a sequence of several smaller changes.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{ApplicativeDo}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{applicativeDo}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{This situation could be improved by centralising information for GHC tool builders. Currently the Haskell wiki hosts a single page ``GHC/As a library'' that gives a few examples of how to run the various stages over a single file~\mbox{%DIFAUXCMD
\citep{ghcAsALibrary}}\hspace{0pt}%DIFAUXCMD
. This is a fine general introduction to working with the GHC API but it does not contain many details about working with the abstract syntax or projects. }\DIFaddend The \DIFdelbegin \DIFdel{under-utilisation of applicatives in Haskellhas not gone un-noticed; \mbox{%DIFAUXCMD
\citep{applicativeDo}}\hspace{0pt}%DIFAUXCMD
...
\DIFdel{algorithm will attempt to insert as many applies into the expression as possible. When the implementation of }\DIFdelend \DIFaddbegin \DIFadd{GHC developer wiki}\footnote{\url{https://ghc.haskell.org/trac/ghc/}} \DIFadd{does contain a great deal of detail about the inner workings of but it is not well organised or easy to find, and a lot of }\DIFaddend the \DIFdelbegin \texttt{\DIFdel{Applicative}} %DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{material there is not directly relevent to tool builders. A centeralised source of Haskell tool building information would help guide people new to the field greatly.
}\DIFaddend 

\DIFdelbegin \DIFdel{There could be multiple ways to desugar a particular function. The }\textit{\DIFdel{applicativeDo}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\section{\DIFdel{Program transformations}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{typeTrans}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Type and transform systems}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Automatic Monadification}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{erwigMonad}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Monadification is developed further by~\mbox{%DIFAUXCMD
\citep{monadification}}\hspace{0pt}%DIFAUXCMD
. This work provides an algorithm for restricted call-by-value monadification as opposed to the semantics style inference rules defined in~\mbox{%DIFAUXCMD
\citep{lammelReuse}}\hspace{0pt}%DIFAUXCMD
. This work targets the lambda calculus extended with case and let expressions. The algorithm from~\mbox{%DIFAUXCMD
\citep{monadification} }\hspace{0pt}%DIFAUXCMD
is very similar to the one implemented in HaRe. It has the same precondition where every call to a monadified function must be fully saturated and it produces the same style of monadification. 
A prototype implementation of this method was produced.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\chapter{\DIFdel{Conclusion}}
%DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
%DIFDELCMD < \label{chp:conc}
%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Summary of Contributions}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This thesis has explored a new category of refactoring for functional programming languages. Data-driven refactorings are transformations that are }\textit{\DIFdel{driven}} %DIFAUXCMD
\DIFdel{by the data types a program uses. The generalisation refactorings, for example, are motivated by a desire to allow programs to be used in more places (e. g. the ``Maybe to MonadPlus'' refactoring) or even to generalise the way a program is evaluated as in the ``monad to applicative'' refactoring where the }\texttt{\DIFdel{Applicative}} %DIFAUXCMD
\DIFdel{typeclass doesn't force sequential evaluation of the arguments to apply (}\texttt{\DIFdel{<*>}}%DIFAUXCMD
\DIFdel{) like bind (}\texttt{\DIFdel{>>=}}%DIFAUXCMD
\DIFdel{), the monad operation, does .
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The primary artifacts of this work are my contributions to HaRe and its API. In particular the following contributions have been made:
}%DIFDELCMD < 
...
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{The design and implementation of a set of data-driven refactorings in HaRe. These refactorings are:
	}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < 		\item %%%
\item%DIFAUXCMD
\DIFdel{Introduce a Type Synonym, section~\ref{introSyn}
		}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Generalise Maybe to MonadPlus/Monad, section~\ref{maybeMonadPlus} and chapter~\ref{generalImp}
		}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{List to Hughes List, section~\ref{listToDlist} and chapter~\ref{generalImp}
		}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Generalise Monad to Applicative, chapter~\ref{chp:applicative}
		}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
...
%DIFDELCMD < \end{itemize}
%DIFDELCMD < \item %%%
\DIFdel{An API for the creation of embeddable types refactorings, chapter~\ref{generalImp}
}%DIFDELCMD < \item %%%
\DIFdel{Enhancements and additions to HaRe's API, see chapter~\ref{generalImp}. The API was extended while developing the refactorings mentioned above.
}%DIFDELCMD < \begin{itemize}
\begin{itemize}%DIFAUXCMD
%DIFDELCMD < 	\item %%%
\item%DIFAUXCMD
\DIFdel{Functions that perform high level, common transformations, that many refactorings must do (e.g. wrapping a syntax element in parenthesis)
	}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Functions for retrieving particular syntax elements from the tree of an entire module such as getting the body of a function based on a position. }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Functions that aid in working with ghc-exactprint and the annotations such as changing the location of a syntax element relative to the prior syntax element or adding new annotations to a syntax element.
}
\end{itemize}%DIFAUXCMD
%DIFDELCMD < \end{itemize}	  
%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend \section{Future Work}
...
, could be used to develop a template language for refactoring Haskell. }\DIFaddend Unfortunately Template Haskell uses it's own abstract syntax for Haskell code which makes it difficult to use for refactoring GHC Haskell. However, there is a plan to change this though it is not finished yet~\citep{ghcTreesGrow}.
\DIFaddbegin 

\textit{\textbf{\DIFadd{Data-Driven refactorings for other languages}}} \DIFadd{- The data-driven refactorings presented in this thesis lean heavily on the types that 
}\DIFaddend 
