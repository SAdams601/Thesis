...
By considering how tools are built we can begin to understand the general toolchain that must be built to implement a program analysis or transformation tool. The language that a tool targets can determine how dependent it will be on third party libraries. Lisp like languages place a strong emphasis on meta-programming, and their internal representation is simply lists so special libraries are not necessary to deal with the abstract syntax\footnote{Or as the creators of clj-refactor prove you don't even have to program your refactoring tool in the same Lisp dialect!}. Wrangler, on the other hand, implements its own generic traversals to help navigate the abstract syntax tree. Haskell is on the heavier side for building tools for. Haskell tools require third party libraries to work with the syntax trees and pretty printing the result. 
...
In general program transformation tools work in three main phases, Figure~\ref{generalToolchain} shows the general toolchain that a tool will need to build. Table~\ref{transformPhases} shows the types of libraries and technologies that are used in each of the phases. 
