...
We begin by discussing a superclass of \texttt{Applicative}, \texttt{Functor}. The \texttt{Functor} typeclass defines a single function that must be implemented, \texttt{fmap}.
...
The \texttt{fmap} function allows for a function to be applied to the contents of the Functor f. One could think of the functor as a context and \texttt{fmap} as a function that allows other functions to run within the \texttt{Functor}'s context. However, what if you wanted to chain together sequences of commands within that context? 

Lets say you have the function and the following two values:
...
\texttt{f} is some functor. You want to get a value of type \texttt{f c} so you do the only thing possible with \texttt{Functor} and use \texttt{fmap}.

\begin{lstlisting}
(fmap g x) :: f (b -> c)
\end{lstlisting}

However, only with \texttt{fmap} there is no way of applying y to the result of the above expression to get a value of type \texttt{f c}. Sequencing commands in this way requires a more powerful abstraction, applicative functors~\cite{realWorldHaskell}. 

In Haskell applicative functors are implemented by the\\ \texttt{Applicative} typeclass. \texttt{Applicative} declares two functions, \texttt{pure} and \texttt{(<*>)}. The types of these two functions are shown below, %in listing~\ref{appTypes} 
where \texttt{f} is the applicative functor. 

\begin{lstlisting}%[frame=tblr,label=appTypes,caption={Applicative's minimal complete definition}]
...
These functions sequence actions and still perform the contextual effects of both of their arguments but discard the value of the first and second argument respectively. These functions are used when some operation affects the applicative context but their result will not affect the overall result of the applicative expression. Consider the following example. 
...
The function \texttt{f} looks up a customer datatype from the \texttt{IO} context and logs all of the customer ids that are looked up. We don't really care what \texttt{writeToLog} returns but we do want the effects that it causes (writing a log file) so we compose the calls to \texttt{writeToLog} and \texttt{getCustomerById} with the \texttt{*>} operator to cause both of the functions' effects to happen but only return the value of \texttt{getCustomerById}.

A consequence of the applicative laws is that every applicative's functor instance will satisfy the following~\cite{control.applicative}: 

\begin{lstlisting}
f <$> x = pure f <*> x
\end{lstlisting}

This means that instead of explicitly lifting every pure function the infix version of \texttt{fmap}: \texttt{(<\$>)} can be used instead. 
...
McBride and Paterson proved that any expression built from the applicative combinators can take the following canonical form~\cite{mcbrideIdioms}:\footnote{This notation is taken from~\cite{mcbrideIdioms} where applicatives were called "idioms", hence the use of "\texttt{is}"}
...
Where the \texttt{is}'s are computations within the applicative context and some may have the form \texttt{pure s} for some pure function \texttt{s}. Due to the rule mentioned at the end of section~\ref{sec:useful} this canonical form can also be expressed using the infix version of fmap \texttt{(<\$>)}. 
...
This is the form that most programs will take when they are refactored from a monadic style. 
 ...
 Context-free parsing is a good use case of the applicative style and many examples in this chapter are taken from parsers defined using the parsec library~\cite{parsec}. The first example of the applicative programming style is a function that parses money amounts of the form:
...
This parser would recognise "\$4.59" or "\textsterling64.56" and parse it into the  data type in Figure~\ref{parseMoney} .
The \texttt{parseMoney} function in Figure~\ref{parseMoney} is in the canonical form as defined by~\cite{mcbrideIdioms}. The pure function \texttt{M} is lifted into the \texttt{CharParser} context and its three arguments are provided by three smaller parsers that handle the currency symbol, the whole amount, and the decimal amount respectively. 

The only difference between \texttt{readWhole} and \texttt{readDecimal} is that \texttt{readDecimal} has to consume the decimal point before reading the number. Instead of duplicating that number code let us perform a small refactoring to lift the parsing of the decimal into the \texttt{parseMoney} function which will allow us to reuse the \texttt{readWhole} function.
...
parseMoney = M <$> parseCurrency <*> readWhole <* char '.' <*> readWhole
 ...
 Here we can see that the result of parsing the decimal point (which would just be the fullstop character) is discarded because of the use of \texttt{<*} rather than the full apply. All of the variations of apply are left associative so the following definition of \texttt{parseMoney} causes a type error.
...
This error can be corrected by parenthesising the subexpression\\ "\texttt{char \textquotesingle.\textquotesingle~*> readWhole}".
...
\texttt{parseStr} does not match the canonical form because no lifted pure function is applied to the rest of the applicative chain. This function could be transformed to canonical form by prepending "\texttt{id <\$>}."
...
To inform the development of this refactoring, is is helpful to understand how the Haskell community has adopted and used the applicative interface. The simplest question we wanted to answer was how many projects are actually taking advantage of the \texttt{Applicative} interface versus using the suggested implementation. When \texttt{Applicative} was made a superclass of \texttt{Monad} an implementation of the \texttt{Applicative} interface was suggested, which allowed programmers to get their existing \texttt{Monad} instances to re-compile successfully with minimum effort. We are interested in how many instances use this suggested implementation.
%% SJT is this two questions or one? Does suggested implementation = default implementation? Need to clarify.
...
Beyond the instances of \texttt{Applicative} that literally define apply to be \texttt{ap} there are several other ways that effectively do the same thing. For example \texttt{Applicative} instances can be automatically derived using the \texttt{GeneralizedNewtypeDeriving} language extension. Another thing that we discovered was several instances duplicated the definition of \texttt{ap} thus:\footnote{Taken from \textit{Sousit} version 0.4}
...
This means that the number of \texttt{Applicative} instances that actually use the suggested implementation is higher than the number found by our search. Note that by defining apply to be \texttt{ap}, this instance of Applicative actually monadic.
...
The computation of \texttt{ap} will require \texttt{m1} to be evaluated strictly before evaluating \texttt{m2}, whereas apply has no such restriction. The arguments to apply could be computed in parallel, as in~\cite{haxl}. Even if not evaluated in parallel, using the \texttt{Applicative} interface rather than the \texttt{Monadic} can keep static values from being re-evaluated because the two sides of the apply operator are not dependent on one another which saves heap space~\cite{errParsers}.

\subsection{Common Implementation Patterns}

In addition to searching how many instances use the standard implementation, we were also curious as to how other instances of \texttt{Applicative} were being implemented. Are there any general patterns that emerge?

Finding general patterns like this is a much more complex task than just searching for calls to \texttt{ap} in the definition of apply. All of the Applicative implementations were written into a report and the authors manually went through the report looking for implementation patterns that cropped up many times. 

The first common pattern we found simply pattern matches on the type's constructor to unwrap inner values and apply them to each other. The following instance declaration, taken from \textit{SGplus} version 1.1, illustrates this approach.
...
Another pattern that we saw used several times involves constraining the inner type to be \texttt{Applicative} as well and just passing off the work to that inner \texttt{Applicative}. The following example comes from \textit{Docker} version 0.3.0.0.
...
This section will describe how the refactoring of monadic code to the applicative style works via a series of examples . Many of these examples are taken from the parser for money amounts and a JSON parser. The full source code for these parsers can be found in~\cite{moneyParse} and~\cite{jsonParser}. 
Consider the example of a parser that parses strings that begin and end with double quotes. 
...
This is a fairly simple function to convert to applicative style. Let's look at another example that adds in more complexity. The function in figure~\ref{pMoney2} is a slight modification of the money parser from Section~\ref{sec:appProgStyle} as well. This version of the parser make the decimal amount optional, so it will recognize both "\textsterling 10.35" and "\texteuro 4".
...
The \texttt{parseMoney} function parses text into the same \texttt{Money} data type from Section~\ref{sec:appProgStyle}. To get the parser to make the decimal amount optional we use the \texttt{option} parser combinator which takes two arguments. Its second argument is parser that it will attempt to apply and if that fails it will return its first argument. 
This new version of \texttt{parseMoney} will be refactored to the applicative style like so:
...
The return statement in the original version calls the function \texttt{M} with the three values that come from the monadic context. Since \texttt{M} is a pure function, it is placed at the front of the chain of applicative operations, composed with \texttt{(<\$>)}. The \texttt{whole} and \texttt{decimal} variables are also being applied to pure functions so again we will compose the pure functions (\texttt{read} in this case) and the effectual computations with infix \texttt{fmap}. 

The where clause in this example has been included for formatting and readability and would not be generated automatically. Even with this clause, however, this function seems quite undesirable compared to the original version. This example would seem to be a bad choice for this refactoring. Fortunately there is a fairly nice solution, just keep refactoring!

We also refactored the inner \texttt{do} block that was part of \texttt{readDecimal} to use the \texttt{Applicative} interface. This final function offers a concise and easy to read alternative to the original function.
...
When there are a large amount of computations that do not affect the final value of the function there can be multiple valid ways to add parenthesis to the function. The definitions of \texttt{objEntry} in Figure~\ref{objEntry2} are equivalent.

Our refactoring will produce the first version of this function, and in general will produce functions with each of the value-producing statements grouped together with the non-value producing statements which are closest to them. Each of these groups will then be surrounded by parentheses, and the groups will be composed using the \texttt{<*>} operator. A more formal description of this is provided in Section~\ref{sec:refacImp}.
...
Many functional refactorings have non-trivial preconditions that must hold before the refactoring can be applied~\cite{refacTools}. Fortunately, the refactoring we are discussing here only has two fairly simple preconditions: first, the target function must be definable with the applicative interface not just the monadic interface and secondly, the order that variables are bound in the \texttt{do} block must correspond to the order in which they are used in the return statement. 

What does this first precondition mean exactly? Where is the line between applicative and monadic? Let's start by looking at the type signatures of the bind and apply functions.
...
The only difference between this version of apply and bind is in the second argument. The second argument of bind is a function that takes in a value of type \texttt{a} and returns an \texttt{m b} whereas apply receives an applicative functor that contains a function from \texttt{a} to \texttt{b}. This means that within a monadic context bind allows access to the pure value returned by a computation in the monad, while all of the arguments to apply are contained within the applicative functor.
What does this mean in practice? Take for example the following function.
...
The function \texttt{f} first parses a number \texttt{n} that corresponds to the number of things that are to be parsed. This function is not definable using applicative functors because we are using the value \texttt{n} as a pure argument to \texttt{parseNThings}. This first precondition will check that any values bound in the target function are not used in the right hand side of any expression other than the \texttt{return} statement.

The second precondition is a bit more intuitive to grasp. Take the following function for example:
...
If we refactor this in the obvious way we would get:

\begin{lstlisting}
f :: Applicative a => a (A,B)
f = (,) <*> getB <*> getA
\end{lstlisting} 

Which would change the type of the program. Then we try: 
...
This will typecheck but this has reversed the order than any effects in the applicative context would occur which makes this transformation not a refactoring because it has changed the behaviour of the program. Our refactoring checks that the variables are bound in the same order that they appear in the return statement which prevents both type errors and changing the order in which effects happen.

\subsection{Additional Refactorings}
\label{sec:otherRefs}
The preconditions described in the previous section, and the second one in particular, may seem overly strict and needlessly limit the scope of code to which the refactoring could be applied. Instead of developing a single monolithic refactoring that contains many separate cases the approach HaRe takes is to develop multiple small refactorings that can be composed together. Composing refactorings together in this way is known as a composite refactoring~\cite{erlangDomain}.
...
Normally the variable \texttt{z} being used again in line five would prevent the function from being refactored. Merging lines four and five into an inline do block would allow this function pass the preconditions.
...
With this modification the function can now be refactored.
...
It is worth noting that further refactoring is possible by applying the generalise applicative refactoring to the inner do block.

\begin{lstlisting}
f = (,) <$> result1 <*> (result2 <* (log <*> result))
\end{lstlisting}

Much like the previous refactoring the extracted do block could be bound to a variable in the \texttt{let} or \texttt{where} if that produces higher code quality.

\subsubsection{Introduce lambdas to reorder statements}

The other precondition to the generalise applicative refactoring is the condition that all bound variables need to be extracted from the monadic context in the order they appear in the return statement. Take the example given at the end of Section~\ref{sec:appPrecons} repeated here:
...
Since \texttt{a} appears before \texttt{b} in the return statement but \texttt{b} is bound before \texttt{a} this function would not pass the second precondition. If this return statement was refactored to include a lambda that flipped the arguments it could pass this precondition.
...
\end{lstlisting}
\caption{Generalise to applicative's top-level function.}
\label{mainFunc}
\end{figure}
...
The refactoring takes  three arguments: the filepath of the file to be refactored, the name of the function to be refactored, and the position (row, column) in the file of the start of the function to be refactored. 

Figure~\ref{mainFunc} shows the main function that drives the generalise applicative refactoring. This function starts by retrieving the parsed AST from the HaRe API with the \texttt{getRefactParsed} function. HaRe provides a few functions like this that retrieve the AST at different stages of the GHC compiler. 

Next, on line four a pure function traverses  the AST and retrieves the subtree that represents the target function. 
From that subtree we pull out two other values, the expression that is on the right hand side of the return statement (e.g. in \texttt{return (a,b)} we are talking about \texttt{(a,b)}), and a list of all of the statements in the do block. Finally we go through all of those do statements and get a list of all the variables that are bound in the do block. 

The rest of this section will go through the checking of preconditions and building the applicative expression more thoroughly. The syntax of the target function has been defined in Figure~\ref{syntax}.
...
Within each cluster we then decide which applicative operator goes between each expression. Since there is only one binding statement per cluster we know that the only options are \texttt{(*>)} or \texttt{(<*)}. We simply go through the list and if an statement occurs before the binding statement we know that it will be followed by a \texttt{(*>)} operator and once the binding statement comes up all the remaining expressions are composed with the \texttt{(<*)} operator. The first cluster from the \texttt{objEntry} example is:

\begin{lstlisting}
spaces *> parseStr <* spaces <* char ':'
\end{lstlisting}

Once every cluster has been refactored separately all of the clusters are wrapped in parentheses and then composed with the full apply operator (\texttt{<*>}), like so

\begin{lstlisting}
    (spaces *> parseStr <* spaces <* char ':') 
<*> (spaces *> many1 digit <* spaces)
\end{lstlisting}
%\captionof{lstlisting}{the fully refactored effectful part of the applicative chain}
...
If the pure expression is just a function like this:
...
then when we build the pure part of the applicative chain we take the expression that is being returned and the list of bound variables. We then remove all of the bound variables from this expression. In the above example this leaves us with just~\texttt{zip lst}, and this expression is what is added to the front of the chain.

\begin{lstlisting}
zip lst <$> getOtherList
\end{lstlisting} 

Once the pure expression has been added the applicative chain is done and we can replace the original function right hand side with this new expression, as shown in Figure \ref{final-fns}.
...
\end{lstlisting}
This is refactored into the following function.
...
Another parser that has several functions that can be refactored is in the implementation of pi-forall, a simple dependently typed language created by Stephanie Weirich~\cite{piForall}. Below is the function that parses a pi-forall module definition.
...
which refactors to the following:
...
The reason to refactor functions like these parsers is mostly for readability. Unfortunately due to the format of this paper the formatting of the functions has been changed to make them more readable. However on a modestly sized monitor, such as the first  author's 13 inch Macbook pro with a resolution of 1280x800, these functions fit onto a single line. In this format the functions can be read from left to right much like the text to be parsed. This makes it very simple to read the function and visualize the text that it is supposed to be parsing. 
