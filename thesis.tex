...
The composition of the set of functions is highly dependent on the relationship between the source and target types and which functions the source program is using. For this reason the implementation of this refactoring in HaRe is as adaptable as possible so that users can customize the set for their own needs (see Chapter~\ref{generalImp}).
...
Figure~\ref{replaceConstructors} shows the top level function \texttt{replaceConstructors} that modifies a target function to replace the \texttt{Maybe} specific values with \texttt{mzero} and \texttt{return}. This function primarily consists of two traversal patterns, the stop top down traversal in \texttt{applyInGRHSs} and the \texttt{everywhereM} in \texttt{runGRHSFun}. The only parts of the function that need to be changed are in the right hand side of the target function, the first traversal (from \texttt{applyInGRHSs}) descends to this level and then stops when the guarded right hand side type is found. From there the other traversal continues down the tree replacing the instances \texttt{Just} and \texttt{Nothing}.
...
\item \textit{Line 8:} Replace the instances of \texttt{Maybe} with the type variable
\item \textit{Lines 9 - 12:} Handle the appearance of the new syntax elements
...
The state consists of three fields, \texttt{funcs} keeps track of the functions that operate over the source type and their equivalent functions that operate on the target type and the names of the projection and abstraction functions. The \texttt{typeStack} field is a stack that keeps track of what changes need to be made further down the abstract syntax tree. Finally, the \texttt{insertAbs} field keeps track of when the abstraction function needs to be applied to certain values.

The call to \texttt{getInitState} on line 16 of figure~\ref{doHughesList} takes in the import declaration of the target type, a list of pairs of strings that represent the equivalent functions over the source and target types, the names of the projection and abstraction functions, the target type's import qualifier if it exists, and the final result type of the target function. 

Once the initial state has been created the transformation is performed in a top down, left to right manner with the call to \texttt{embRefact} on line 17 of figure~\ref{doHughesList}. As seen in figure~\ref{embRefact} the definition of \texttt{embRefact} simply calls the API function \texttt{modMGAltsRHS} which simply applies a function that modifies an expression to each of the right hand side values of the given function binding. The ``real'' work of the transformation occurs in \texttt{doEmbRefact} which is defined in figure~\ref{doEmbRefact}.
...
\subsection{The type stack}

The type stack is part of the refactoring's state and is of type \texttt{[Maybe GHC.Type]} each member of the stack represents the type that a sub tree must be changed to. The stack will be initialised with a single value ``\texttt{Just (DList a)}'' where \texttt{a} is the type of list from the source function this is because the result type of the entire syntax tree needs to be changed to \texttt{Just (DList a)}. The stack keeps track the ``goal types'' of each sub-tree. As an example of this process look to the simple abstract syntax tree that represents a call to the \texttt{replicate} function with the variables \texttt{n :: Int} and \texttt{x :: Char}. 

If the goal was to transform this tree to be of type \texttt{DList Char} the stack would be initialised to be \texttt{Just (DList Char)} because this is the goal result type of the entire syntax tree. The tree's type is determined by the result type of the left most leaf of the tree so the transformation will descend there and replace the call to \texttt{replicate} with one to \texttt{DList.replicate}. After this replacement is made the transformation may have caused a type error in the program because \texttt{replicate}'s arguments are not necessarily the same type as \texttt{DList.replicate}'s. The argument types of the two functions are compared and the results of the comparison are pushed onto the stack. In this case the arguments for both versions of replicate are actually the same type so \texttt{[Nothing, Nothing]} is pushed onto the stack. When the transformation gets to the \texttt{HsVar} leaves for \texttt{n} and \texttt{x} respectively it pops the top of the stack off but since there is just a \texttt{Nothing} no more changes are needed in that subtree.



\begin{figure}[t]
	\begin{center}
		\includegraphics[scale=.5]{graphVis/Chapter4/simple.png}
	\end{center}
	\caption{A simplified syntax tree of a call to \texttt{replicate :: Int -> Char -> [Char]}.}
	\label{replicateAST}
\end{figure}

In the case that there is a \texttt{Just} value at the top of the stack then the transformation will attempt to change the leftmost leaf so that the subtree's result type matches the type on the top of the stack.

\subsection{Traversing an expression, the implementation of \texttt{doEmbRefact'}}

Though the top level \texttt{doEmbRefact} function determines if a subtree should be changed at all \texttt{doEmbRefact'} determines how the transformation travels through the subtree and the changes that occur based on the type of the current abstract syntax node.

\begin{enumerate}
\item Explain cases of \texttt{doEmbRefact'}
\item Explain todo work
\end{enumerate}

\subsection{Future work in the embeddable type refactoring system}
