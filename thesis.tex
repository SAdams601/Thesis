...
Scrap your boilerplate is not the only generic programming library for Haskell, in fact in the past 15 years many other libraries have been released~\citep{comparingGenerics}. In addition to scrap your boilerplate HaRe continues to use Strafunski-StrategyLib the generic programming library that HaRe originally used\footnote{See \ref{strafunskiSection}}~\citep{strafunski}. HaRe continues to use Strafunski because it supports several unique traversal combinators that are very useful. Much like SYB, Strafunski defines traversals by whether they unify or preserve the types of the target datatype. 
