...
The final section of this chapter will discuss the technology that a refactoring tool for Haskell is dependent on. In particular, Section~\ref{buildTool} will cover language front ends for Haskell, generic programming libraries, and other common dependencies that refactoring tools require.  
...
\section{Building a refactoring tool for Haskell}\label{buildTool}

Well designed software should be designed to be as focused as possible. A web app should not also implement its own http server. There are already plenty of http servers already written, and unless absolutely necessary a web app should rely on one of them rather than reimplementing work that others have already performed. Refactoring tools also have to perform tasks that are better suited to being handled by dependencies. In particular generating abstract syntax trees of the target language and traversing those trees can be very arduous without the assistance of third party libraries.

The final section of this chapter on related work will cover some of the dependencies that support the development of Haskell refactoring tools and specifically HaRe's current dependencies. Language front ends provide access to the internal structure and components of a programming language. Front ends allow third party applications reuse the components built for interpreting and executing the target programming language. Section~\ref{frontEnds} discusses the front ends that HaRe has used to to interact with GHC. Section~\ref{genericProgrammingLibs} describes the other major dependency that HaRe and other refactoring tools depend on, a generic programming library. Generic programming libraries help minimise the amount of code it takes to work with complex recursive structures such as abstract syntax trees. 

\section{Language Front Ends for Haskell}\label{frontEnds}

	To refactor programs, refactoring tools must represent these programs in a machine readable way. This is what a language's compiler and runtime system must also do. A language front end gives access to these components. This section will describe two front ends for Haskell, Programmatica and the GHC API.   

\subsection{Programatica}

Programatica was a project at the OGI School of Science and Engineering to build tool support for validating Haskell programs~\citep{programaticaTools}. The Programatica team open sourced their frontend so that other tools could also use it~\citep{refacWebsite}. When development on HaRe started there was no API to access the internals of GHC so Programatica was chosen as HaRe's front end. Programmatica was picked over other available front ends because it was the simplest front end that supported the full Haskell 98 standard and a number of its extensions~\citep{huiqingThesis}. Programatica's Haskell front end is broken up into multiple components including a lexer, a parser, an abstract syntax tree data type, a module system, a type checker, and a pretty printer.

Unfortunately the Haskell 98 standard has been superseded by the Haskell 2010 standard. In addition to the Haskell 2010 standard the Glasgow Haskell Compiler has become the dominant implementation of Haskell and it supports extensions to the language standard. HaRe required a new front end to continue to support the Haskell community. GHC now comes with a front end built in known as the GHC API. This front end is what HaRe currently depends on.
...
\section{Generic Programming}\label{genericProgrammingLibs}

The previous section described just a small subset of the abstract syntax of the GHC API. Even Programatica, which was selected for it's simplicity, had an abstract syntax made up of 20 different data types that had 110 data constructors in total~\citep{programatica}. Working with the large data types that represent a programming language directly would introduce a large amount of ``boilerplate'' code into a program. Boilerplate code is the repetitive code that traverses a large structure. For example, the \texttt{HsExpr} type has over 30 constructors, searching for a particular variable within a single expression would require writing a function that would terminate if it reaches an \texttt{HsVar} constructor with the desired name, otherwise it would have to pattern match all the other constructors to recursively descend their child expressions. This boilerplate code is highly verbose and difficult to maintain which makes it unwise to have it in a code base. 

Generic programming libraries help eliminate this type of code. There are many generic programming libraries for Haskell but this section will describe two of them: Strafunski-StrategyLib and Scrap Your Boilerplate (SYB). These two libraries are both used by HaRe.~\cite{comparingGenerics} provides a more comprehensive overview of the generic programming libraries for Haskell.  

\subsection{Strafunski-StrategyLib}

The Stratego/XT library was one of first systems for programming tree transformations in a systematic way~\citep{stratego}. Stratego developed the idea of a transformation strategy. A strategy is the combination of a term rewriting function and a traversal function. A traversal function describes how the rewriting function should be applied to a tree of terms. Stratego provides combinators that help construct term rewriting functions and tree traversal functions \citep{stratego}.

Stratego is an untyped transformation system and so was unsuitable for working with the statically typed Haskell. Strafunski was ``largely inspired'' by Stratego, but for a statically typed context~\citep{strafunski}. Strafunski was first developed in~\cite{strafunskiOrig} but much of the work that HaRe depends on was first described in~\cite{strafunski}.
...
This type of generic traversal is very common in HaRe because traversals often need to make use of a refactoring's stored state or run something from the GHC API which needs to be computed in an instance of \texttt{GhcMonad}, while also modifying the abstract syntax. The GHC API's operations all work within the \texttt{GhcMonad} that provides the features GHC needs to compile a Haskell source file such as IO, logging warning, exception handling, and keeping track of the compilation session~\citep{ghcApi}.
...
The final sections of this chapter discussed the tools that are used to implement HaRe, a language front end and generic programming libraries. With this background the thesis will continue in the next chapter by describing how refactorings are implemented in HaRe. 
...
This chapter begins, in Section~\ref{hareOrig}, with a brief discussion of the original implementation of HaRe. Most of HaRe's current dependencies were described in Chapter~\ref{chp:related}, however Section~\ref{ghcExactprint} describes, \textit{ghc-exactprint}, the pretty-printer HaRe uses to output the modified program~\citep{exactprint}. This pretty printer was developed by Alan Zimmerman and Chris Pickering specifically for HaRe but a description of how it works has not appeared in the literature so it has been placed into this chapter. Finally the chapter will conclude with a description of the current implementation of HaRe in Section~\ref{hareCurr}.

\section{The original implementation of HaRe}
\label{hareOrig}

HaRe was originally implemented using the Programatica language front end for Haskell 98 and Strafunski-StrategyLib provided generic traversals which made handling Programatica's syntax tree much easier, these libraries were discussed in Chapter~\ref{chp:related}. This section will describe the types of refactorings that the original HaRe provided. Section~\ref{hareApi} will also describe the API the HaRe provided. HaRe has always been an open source project that wanted people outside of Kent to contribute to, to help facilitate this HaRe provided an API for refactoring and program analysis. This API helped people develop their own refactorings without needing to understand the entire codebase, only the higher level functionality that the API provided. 
...
Even with these failures we were still able to parse more than 90\% of the Haskell source files contained in Hackage. From the successfully parsed files a total of 2,349 \texttt{Applicative} and 2,098 \texttt{Monad} instance declarations were found.

\subsection{Search Results}
  
The found \texttt{Applicative} instances in 826 cases were defined using the suggested implementation of \texttt{(<*>) = ap}.

Beyond the instances of \texttt{Applicative} that simply define apply to be \texttt{ap} there are several other ways that effectively do the same thing. For example \texttt{Applicative} instances can be automatically derived using the \texttt{GeneralizedNewtypeDeriving} language extension. 
...
The definition of \texttt{ap} (see figure~\ref{apDefn}) forces \texttt{m1} to be evaluated strictly before evaluating \texttt{m2}, whereas apply has no such restriction. The arguments to apply could be computed in parallel, as in~\citep{haxl}. Even if not evaluated in parallel, using the \texttt{Applicative} interface rather than the \texttt{Monadic} can keep static values from being re-evaluated because the two sides of the apply operator are not dependent on one another, which saves heap space~\citep{errParsers}.
