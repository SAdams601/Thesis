...
The second precondition of this refactoring is that the target functions cannot be called outside the scope of the refactoring. Once a function has been monadified it's result cannot be used within a pure function any longer because there is no way to retrieve the result of a monadic computation from its context.
...
The simplest case of this refactoring is when the target function is defined using only pure functions then the refactored version of the function is just \texttt{return} applied to the original body of the fuction. Otherwise the refactoring works over the body of a target function in three steps:

\begin{enumerate}
\item Replace any right-hand sub-trees that are calls to functions that are targeted in the refactoring with variables in a top-down left to right manner. For example the the definition \texttt{(f x = f (g x))},  becomes \texttt{(f v1)} with \texttt{v1} bound to \texttt{(g x)}, assuming \texttt{f} and \texttt{g} are targets of the refactoring.
\item If the expression is not a call to one of the target functions then it needs to be passed to return. From the example in step one, the body of \texttt{f} had become \texttt{(f v1)}. Since \texttt{f} is one of the target functions this expression is monadic and doesn't need to be returned.
