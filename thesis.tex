...
The previous section discussed a refactoring that is most useful in the early stages of development when the details of data representation are in their infancy. It's also quite possible that abstractions will become quite developed before it's understood that a change is required.

\begin{itemize}
\item Motivation: sometimes we decide on wrong abstraction and need to change mid development
\item Informally isomorphic types, aka sets of functions that are equivalent
\item Section: The simplest isomorphic refactoring Maps.Lazy to Maps.Strict
\item Section: What are hughes lists
\item Section: Lists to Hughes List refactoring
\end{itemize}
