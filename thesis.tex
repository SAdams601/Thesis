...
The top level datatype for all of GHS's abstract syntax is \texttt{HsSyn}~\citep{ghcDesign}. \texttt{HsSyn} is parameterised by some identifier; each compiler stage produces a different type of identifier with the additional information that stage produces. For example the typechecker takes in an AST parameterised by \texttt{Name} and returns an AST parameterised by \texttt{Id} which is a \texttt{Name} with additional type information.
...
renameXVar = everywhere (mkT rename)
	...
	Type extension allows for the \texttt{rename} function to work over many types. After type extending \texttt{rename} it will work as expected when applied to an argument of type name, and as the identity function when applied to any other type.
	
	The \texttt{everywhere} function is this traversal's combinator, \texttt{everywhere} applies the extended function to every node in the tree. Finally as you can see in the declaration of \texttt{Expr} that it derives both the Typeable and Data classes. A member of the Typeable class has defined a generic representation of itself and members of the Data class implement generic folding operations. Put together these two classes are what allow generic traversals. 
...
Strafunski has two different types of strategy combinators, type preserving and type unifying~\citep{strafunski}. A type preserving, or ``\texttt{TP}" strategy when applied to some term of type \texttt{t} will return type \texttt{t}, whereas type unifying (\texttt{TU}) strategies return a different type. All of Strafunski's combinators are qualified with either a \texttt{TU} or \texttt{TP} postfix. In general the type preserving strategies are used for transformation whereas type unifying strategies are for analysis. 
