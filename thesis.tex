...
First-class functions mean that functions are allowed to be treated like any other data type. First-class functions can be passed-to or returned-from from other ``higher-ordered'' functions. There is also a heavy emphasis on functions remaining ``pure'' that is without side-effects, though functional languages do provide ways to use IO or state it is emphasised that functions should remain pure if at all possible. Finally iteration is accomplished through recursion rather than looping.
...
Lazy evaluation means that Haskell expressions are not evaluated when they are passes as a parameter, but rather when that value is used. For example, in the Haskell function \texttt{f}, in figure~\ref{lazyY}, 
...
\begin{itemize}
	\item Extending the HaRe API to better support data-driven refactorings. These refactorings are complex and require more information from the abstract syntax of GHC than prior refactorings in HaRe. The contributions to the API were focused on analysis of the types of nodes and creating a higher level interface for common small expression level transformations.
	\item The design and implementation of a set of data-driven refactorings. This is a new category of refactorings that haven't been developed for Haskell previously.
\end{itemize}
...
This chapter will cover the history of HaRe, the technologies it depends on, and the current implementation of HaRe. Specifically there will be an overview of the GHC API~\citep{ghcApi}, the generic traversal library Scrap Your Boilerplate~\citep{syb}, and ghc-exactprint~\citep{exactprint}. This chapter will also describe how the inner workings of HaRe are implemented and the functions that compose its API. Finally it will describe the design and development process of implementing refactorings for HaRe.

\textbf{Chapter~\ref{chp:ddRefs}: Data-Driven Refactorings}

Chapter~\ref{chp:ddRefs} will be an introduction to data-driven refactorings. The chapter begins with a discussion of this type of refactoring for object-oriented languages. The rest of the chapter will describe data-driven refactorings for the Haskell programming language. This begins with a description of the ``introduce a type synonym'' the ``renaming'' refactoring of data-driven refactorings. The other two refactorings covered in this chapter are the ``generalising maybe'' and ``list to hughes list'' refactorings.

The ``generalising maybe'' refactoring rewrites functions that use the concrete type of \texttt{Maybe} to instead use the operations provided by the typeclasses it implements \texttt{Monad} and/or \texttt{MonadPlus}. The final refactoring describe by this chapter is the ``list to Hughes list'' refactoring. Hughes lists (also known as difference lists) are an alternative implementation for lists that support $O(n)$ time appends. This refactoring takes functions that use that standard list implementation and rewrites the function to use Hughe lists instead. The approach for this refactoring is applicable between any two types that are ``reversibly embeddable'' a concept that will be defined as well.

\textbf{Chapter~\ref{chp:applicative}: Generalising Monads to Applicative}

This chapter will present another generalisation refactoring. Applicative functors are a, relatively, new addition to the Haskell environment. Applicative functors are an interface for sequencing effectful computations. Currently the Haskell community predominantly uses the monadic interface for effects. This chapter will describe the design and implementation of a refactoring for taking a monadic \texttt{do} statement and transforming it to use the \texttt{Applicative} (the Haskell typeclass for Applicative functors) interface instead.
...
Chapter~\ref{chp:monadification} describes the monadification refactoring. Monadification is the process of introducing monads into pure code. Monads are the standard way for effects to be used in Haskell and so a refactoring to automatically add them is very useful to the community. There are many styles of monadification and this chapter will describe several of them. Finally it will discuss the implementation of the monadification refactoring in HaRe.

\textbf{Chapter~\ref{chp:related}: Related work}

The final content chapter will discuss the literature related to this thesis. This chapter covers other refactoring tools for other functional languages and object-oriented languages. This chapter also discusses multiple projects that are using refactoring in unique and interesting ways such as introducing parallelism, and non-traditional programming languages. Next the chapter describes work on developing syntactic sugar for applicative functors. Finally there is a brief discussion of type changing program transformation systems.
...
\section{``Lists to Hughes List'' Refactoring}\label{listToDlist}
...
\item The design and implementation of a set of data-driven refactorings in HaRe. These refactorings are:
	\begin{itemize}
		\item Introduce a Type Synonym
		\item Generalise Maybe to MonadPlus/Monad
		\item List to Hughes List
		\item Generalise Monad to Applicative 
		\item Monadification		
	\end{itemize}
\item An API for the creation of embeddable types refactorings
\item Enhancements and additions to HaRe's API. The API was extended while developing the refactorings mentioned above.
\begin{itemize}
