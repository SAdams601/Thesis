...
The typed lambda calculus with polymorphic let expressions makes up the core of Haskell\comment{citation for this??}. With this section the refactoring supports the transformation of the core of Haskell. However, this refactoring outputs monadic code composed with the bind operator, which is probably not the most popular way to write monadic code. To better support monadic programming Haskell gives monads their own syntactic structure known as do-syntax. The next section will discuss a separate refactoring for introducing this syntax into the target function.
