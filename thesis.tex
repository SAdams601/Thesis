...
This chapter reviews current work in both the refactoring and other areas of the literature. The chapter is divided into three sections. The first section~\label{funcTools} will describe work done in the area of tooling for functional languages such as HLint~\citep{hlint} a tool that suggests improvements to Haskell code and Wrangler~\citep{wrangler} a refactoring tool for Erlang. The next section, \ref{otherTools}, will discuss refactoring for non-functional programming languages such as the tools packed into most modern IDEs . Finally section~\ref{typeTrans} covers work done on program transformation, with a focus on type changing transformations.
...
\item OCaml refactorer - ROTOR from this years OCaml symposium.
\end{enumerate}


\section{Refactoring other langauges}\label{otherTools}
