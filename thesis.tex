...
\comment{This chapter will introduce the most basic concepts of this thesis. In particular it will discuss, refactoring in general, functional refactoring, and the Haskell programming language. It will also state the contributions of this research and outline the rest of the thesis.}
...
\todo{Copy and paste relevant material from mini-thesis}


Implementing an automated refactoring system requires two primary dependencies, a frontend for the language that your refactorer targets, and a generic programming library. A language frontend is required for your system to analyse and modify source code. A generic programming library is required to traverse the complex structure of a real world programming language's abstract syntax tree. This chapter will summarise the technologies I examined this year, two Haskell frontends, Programatica~\citep{programatica} and the GHC API~\citep{ghcApi}, and two generic programming libraries, Strafunski StrategyLib~\citep{strafunski} and Scrap Your Boilerplate~\citep{syb}. Finally this chapter will discuss some of the other projects in the fields of automated refactoring and program transformation.

\section{Haskell Frontends}
At its most basic level refactoring is about modifying source code. Without some way to represent source code within a separate program an automatic refactorer would not be possible. A programming language frontend provides this representation. This section will look at two Haskell frontends Programatica and the GHC API.

\subsection{Programatica}
The original implementation of HaRe used Programatica as its frontend. Programatica was selected for use in HaRe because it supports the full Haskel 98 standard and at the time it reported more information than other frontends~\citep{huiqingThesis}.

Programatica's abstract syntax for Haskell 98 uses 20 data type and 110 data constructors~\citep{huiqingThesis}. This abstract syntax is produced by Programatica's parser, which is based on Haskell 98's HsParser. Instead of using HsParser's lexer Programatica provides it's own. Programatica's lexer produces a token stream that includes a token's position in the source. The type of the lexer is:

\begin{verbatim}
     lexer :: String -> [(Token,(Pos,String))]
\end{verbatim}

The input string is the program source, the token data type classifies different kinds of tokens, pos represents the token's position in the source file by row and column numbers, and the final string is the contents of the token. From this lexer the parser produces an abstract syntax that includes source locations for each identifier~\citep{huiqingThesis}.

In addition to providing an AST with source location annotations, Programatica can also generate a syntax tree annotated with type information. This is provided by Programatica's type checking system. In this variation of the syntax tree declaration lists have been annotated with the kinds and types of the entities defined in the list, and all applications of polymorphic functions have been decorated with the instantiation of the type variables~\citep{huiqingThesis}.

\subsection{The GHC API}

The GHC API exposes the entire inner structure and working of the GHC to users~\citep{ghcDesign}. The GHC API isn't so much a layer over the top of the internal workings of the compiler rather than the compiler itself. The GHC was built as a set of smaller components rather than as a monolithic executable. What people think of as the GHC executable is just a lightweight main function that ties together the smaller components~\citep{ghcDesign}. These components are what constitute the GHC API. This subsection will discuss these components and how the GHC identifies definitions. An in-depth discussion of how the AST is structured can be found it section~\ref{ghcAst}.

\subsubsection{Compiler stages of the GHC}

I have listed the first few GHC compiler steps in figure~\ref{compilerStages} because they are the stages that my research interacts with. The full GHC pathway diagram can be found in~\citep{ghcDesign}. The label after each stage of the compiler indicates the type of the AST that is produced by that stage. After desugaring the GHC has fully translated Haskell into an intermediate language known as Core~\citep{ghcDesign}. 
...
The first step in the GHC pathway is the parser which translates Haskell source code into an AST of the type $HsSyn~RdrName$. $HsSyn$ is the top level abstract syntax datatype; it is parameterised by the type of identifier that it contains. $HsSyn$ is a large collection of datatypes that, together, describe the full abstract syntax of Haskell. At last count there are 52 different datatypes that make up $HsSyn$~\citep{hssynWiki}.

\subsubsection{GHC's Identifiers}

The GHC has five different identifier types~\citep{ghcHackage}. The main pathway only uses three identifiers $RdrName$, $Name$, and $Id$, the other two types of identifiers are records in the other identifier types. $OccName$ is the most basic name type the GHC uses. An $OccName$ just consists of a string representing the name of the identifier and the namespace it exists in. $OccName$ is used by all the other name types.

An $RdrName$ is the type of identifier that the parsed AST uses. It consists of an $OccName$ paired with module information, the module where the identifier is defined and the module that the identifier is imported from. In the tree that results from renaming the $Name$ identifier is used. A $Name$ has its scoping and binding information resolved. $Name$ also contains a $Unique$ type. A $Unique$ is a datatype for performing fast ordering and equality tests and is used throughout the GHC~\citep{ghcHackage}. After type checking the AST uses $Id$s. $Id$s contain type information in addition to the data $Name$s already store. 
...
\todo{This needs cleaning up...}

Work on HaRe was started at the University of Kent in the Spring of 2003~\citep{refacWebsite}. Since then it has formed the basis for the work Huiqing Li did for her PhD thesis~\citep{huiqingThesis}, and many other publications since then, including but not limited to~\citep{brownTypes,formalisationRefac,hareApi,haskellClones,brownThesis}. 

Initially HaRe supported the full Haskell 98 standard and was integrated into both the Vim and Emacs text editors~\citep{huiqingThesis}. However, Haskell 98 is quite outdated and the Haskell community has adopted the Glasgow Haskell Compiler (GHC) as the language's de facto standard. The GHC uses the newer Haskell 2010 standard along with several language extensions. Work is currently ongoing to port HaRe from Haskell 98 to the GHC.   

In \citep{huiqingThesis} the required dependencies of a Haskell-in-Haskell refactorer are said to be: 

	\begin{itemize}
	\item A lexer
	\item A parser
	\item A type-checker
	\item A module analysis system
	\item A pretty printer
	\item A generic way to traverse an abstract syntax tree
	\end{itemize}
	
The original implementation of HaRe fulfilled these dependencies with two libraries, Programatica and Strafunski-StrategyLib. As discussed previously in section~\ref{strafunskiSection}~Strafunski-StrategyLib allows for generic traversals over typed data. Strafunski was used in HaRe to traverse and transform the Haskell AST without having to write and maintain all of the boilerplate code normally required to work with the large and complex tree that represents Haskell source code. Programatica provides the other five requirements for HaRe~\citep{huiqingThesis}. More in-depth discussion of these tools is not in this section as both of these tools were covered in previous chapters.


\subsection{HaRe's original refactorings}\label{origRefactorings}

HaRe's original refactorings fall into three categories, structural, module, and data-oriented refactorings. These original refactorings were chosen to be basic yet still useful, and for their ability to give insight into the issues surrounding implementing an automated refactoring tool~\citep{huiqingThesis}. The following is a brief discussion of these three categories and some sample refactorings within them.
...
\subsubsection{Data-Oriented Refactorings}
...
\comment{
\subsection{Multi-module refactoring}\label{multiModRefac}

In its initial release HaRe only supported a small set of structural refactorings. Even at this initial stage it was still recognised that, for HaRe to continue to grow, it was going to have to be aware of modules~\citep{toolSupport}. For example, in a multi-module program if a renaming takes place any other modules that reference the renamed identifier will have to be updated to use the new name as well.

A multi-module refactoring is done is two separate steps: transforming the module where the refactoring is initiated (this module is referred to as the \textit{current module}) and transforming the module which import the current module (these modules are called \textit{client modules}). 

When performing a multi-module refactoring HaRe needs to extend both side-condition checking and program transformation. During a renaming refactoring, as mentioned previously, any client modules that reference the renamed definition need to be modified to use the new name; additionally the checks that detect issues such as name clashes and captures need to be performed in every client module as well~\citep{huiqingThesis}.

In reality just extending the side condition and transformation functions to operate over client modules is not the most effective implementation. One of the general strategies HaRe has adopted to improve efficiency is reducing the amount of AST traversal that each refactoring does~\citep{huiqingThesis}. The most obvious way this happens is that side-condition checking in client modules happens during program transformation, rather than happening previously in its own step.
}
...
\section{HaRe and the GHC}

As early as 2005 the Glasgow Haskell Compiler (GHC) was becoming the de facto language standard for Haskell. At that point HaRe underwent an evaluation of the work required to support the GHC~\citep{portingHare}. Additionally HaRe's Haskell front-end, Programatica, had two major limitations. The first was type checking with Programatica is slow and HaRe's implementors wanted to make use of type information. Also Programatica only supported Haskell 98. Many non-trivial Haskell projects used various extensions to this standard and being unable to support these extensions reduced the usefulness of HaRe to practising programmers~\citep{portingHare}.

Currently work is being done on moving HaRe to work with the GHC API. This work is being lead by a developer named Alan Zimmerman. HaRe is being developed under an open source licence on the website Github\footnote{HaRe's repository: https://github.com/alanz/HaRe}.

This section describes the current state of HaRe and how it has been updated to use the GHC API. 

\subsection{HaRe's Utilities and API}

HaRe's utilities closely resemble the functionality available in the initial version of HaRe's API as described in~\citep{hareApi}. In addition to source code analysis and transformation functions HaRe also includes a refactoring monad of type:

\begin{verbatim}
     type RefactGhc a = GHC.GhcT (StateT RefactState IO) a
\end{verbatim}

The RefactGhc monad helps keep track of a single refactoring. RefactState is the data type that stores the current state of refactoring and updates the token stream to reflect changes made to the AST. This is transformed into a state monad through the use of \textit{StateT} to allow for the refactoring state to be updated as the refactoring executes. Finally everything is added to a GHC monad. The GHC monad provides all the other features required to make GHC API calls~\citep{ghcMonad}. People who wish to use the HaRe API will need to become familiar with RefactGhc as much of the API operates within the monad.

All of HaRe's utility functions are grouped into separate submodules depending on purpose. To make HaRe's API easier to use in a client module the \textit{Language.Haskell.Refact.API} module was created. This module imports and re-exports the entire HaRe API. This grants access to all of the API with only a single import.

\subsection{HaRe's refactorings}

Probably the biggest difference between pre-GHC HaRe and the current codebase is the number of refactorings each supported. The older implementation of HaRe currently supports 51 different refactorings across the three refactoring categories described in subsection~\ref{origRefactorings}. The current codebase of HaRe currently supports five different structural refactorings.

\begin{itemize}
	\item Convert an if expression to a case expression
	\item Duplicate a definition
	\item Move a definition up a level
	\item Swap arguments
	\item Renaming
\end{itemize}   

Adding more refactorings to HaRe has recently taken a back seat to the development of HaRe's infrastructure. Once the infrastructure becomes more stable expanding the library of available refactorings will become a priority. 

\todo{incorporate the description of HaRe that I wrote for IFL paper.}
...
\todo{\textbf{Add section about ML languages.}}
