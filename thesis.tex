...
Since the result type is the only type of \texttt{enumerate} that is affected the refactoring needs to modify the function definition so that the right hand side of its definition is of type \texttt{DList a} instead of \texttt{[a]}. It does this by traversing the abstract syntax tree in a depth first manner starting with the left hand sub-tree. 

\comment{MAke the first example be a case where the argument becomes DList as it is a bit simpler}
